...
# @author Lovas Szilard <lovas@highfinesse.de>, Lukas Lankes <l.lankes@fz-juelich.de>
# @date 2018.09.14
# @version 0.2
#
...
# 2024.11.11
# Error handling, logging and minor refactoring (l.lankes)
#/
...
__all__ = ["dll", "load_dll", "close_dll"]
...
def load_dll(path_to_dll: str = None, reload: bool = False):
    global dll

    if dll is not None:
        if not reload:
            # Return dll as it is already loaded
            return dll

    if path_to_dll is None:
        # Use wlmData.dll as default path
        path_to_dll = "wlmData.dll"

    if not os.path.isfile(path_to_dll):
        # Use path as passed by the caller
        real_path_to_dll = path_to_dll
    else:
        # Find DLL on search paths
        real_path_to_dll = ctypes.util.find_library(path_to_dll)
        if real_path_to_dll is None:
            raise FileNotFoundError("Could not find wlmData library: " + path_to_dll)
        logger.debug("Found DLL: " + real_path_to_dll)

    # Load DLL with ctypes
    logger.debug("Loading library: " + real_path_to_dll)
    dll = ctypes.WinDLL(real_path_to_dll)
